...
#[get("/kt2_public")]
async fn kt2_public(public: web::Data<Arc<PublicKey>>) -> Result<impl Responder, Error> {
    Ok(web::Json(bs58::encode(public.bytes).into_string()))
}

#[post("/create_vote")]
async fn create_vote(
    options: web::Json<(String, String, Vec<String>)>,
...
async fn submit_ballot(
    body: web::Json<(VoteID, Ballot, Option<[u8; 32]>, [u8; 32])>,
    db_tx: web::Data<DbGetter>,
    ...
    Ok(web::Json((ballot_id.as_string(), kt2_str)))
}
